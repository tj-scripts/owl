...
def create_role_playing_model(
    ...
    if role_name not in ["USER", "ASSISTANT", "BROWSING", "PLANNING", "IMAGE"]:
        raise ValueError(f"Invalid role name: {role_name}")
    
    # Check if default role name is valid
    if default_role_name not in ["LLM", "VLM"]:
        raise ValueError(f"Invalid default role name: {default_role_name}")

    # Get model type from environment variable
    model_type=os.getenv(f"{role_name}_ROLE_API_MODEL_TYPE", os.getenv(f"{default_role_name}_ROLE_API_MODEL_TYPE", (default_llm_model_type if default_role_name == "LLM" else default_vlm_model_type)))
...
def construct_society(question: str) -> RolePlaying:
    ...
    models = {
        ...
        "browsing": create_role_playing_model("BROWSING", "VLM"),
        "planning": create_role_playing_model("PLANNING"),
        "image": create_role_playing_model("IMAGE", "VLM"),
    }
